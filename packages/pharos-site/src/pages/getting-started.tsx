...
<>
  ...
  <br />
  <p>
    Once installed, the `node_modules/` directory should contain directories similar to following
    structure:
  </p>
  {/* // ```
  ...
  <p>
    You will first need to provide the scoped custom element registry to your application. This is
    generally achieved by including this script tag in the head of your application.
  </p>
  {/* // ```jsx
  ...
  <p>
    ...
    registry](https://developer.mozilla.org/en-US/docs/Web/API/CustomElementRegistry) for them to be
    usable in your code. To register components, you can use the `registerComponents` utility to
    define all the Pharos components your application uses, with a consistent scoping prefix:
  </p>
  {/* // ```javascript
  ...
  <p>
    ...
    entrypoint and define the custom element with a tag name in the form of `{app / bundle}-pharos-
    {component}` and a trivial subclass that extends the Pharos class wrapped in the
  ...
  <p>
    The following code shows how to use Pharos's button component with **React** to produce a basic
    button, as seen below the code
  </p>
  {/* // ```jsx
  ...
  <p>
    Similarly, the following code shows how to use Pharos's button component with **Vue** to produce
    a basic button, as seen below the code
  </p>
  {/* // ```
  // <template>
  //   ...
  ...
  <p>
    ...
    component, is crucial in using the Pharos design system. Pharos components are fully documented
    to allow IDEs to take advantage of intellisense features, which in turn allows you to browse
    through props.
  </p>
  <p>
  ...
  <p>
    As part of the package, Pharos includes a few curated fonts that are integrated into components.
    To allow consistency between components and other elements, you should use these fonts for other
    parts of the site. If you want to read more about Pharos' typography, including guidelines for
    font usage scenarios, head over to the{' '}
    <PharosLink href="http://localhost:8000/brand-expressions/typography">typography</PharosLink>{' '}
    page.
  </p>
  <p>
    To use the correct fonts, start by importing them by importing
    `@ithaka/pharos/lib/styles/fonts.css` at the top level of your web app. Once imported, set the
    root body font and styles by importing `@ithaka/pharos/lib/styles/typography.scss`. If there are
    places where the body font is being overwritten, apply the font wherever the style is being
    overwritten by setting the `font-family` property to include either `GT America Standard`
    (generally for body) or `Ivar Headline` (generally for headers).
  </p>
  <p>
    The following examples use inline styling, but the same principles remain for other methods of
    styling.
  </p>
  ...
  <p>
    ...
    including spacing, size, and colors. These design tokens are also provided to developers using
    Pharos in order to provide assistance in creating a consistent look and feel in pages where both
    Pharos components and other elements exist.
  </p>
  <p>
    To start using the design tokens, import the `@ithaka/pharos/lib/styles/_variables.scss` file
    and use the desired token in the styles for your web app. You can inspect the imported file to
    see all the tokens and their corresponding values.
  </p>
  <br />
  <br />
  <PharosHeading level={2} preset="2">
