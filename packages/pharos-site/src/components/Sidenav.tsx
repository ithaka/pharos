...
const Sidenav: FC = () => {
  ...
  useLayoutEffect(() => {
    (async () => {
      ...
      const { PharosSidenav } = await import(
        '@ithaka/pharos/lib/react-components/sidenav/pharos-sidenav'
      );
      const { PharosSidenavSection } = await import(
        '@ithaka/pharos/lib/react-components/sidenav/pharos-sidenav-section'
      );
      const { PharosSidenavLink } = await import(
        '@ithaka/pharos/lib/react-components/sidenav/pharos-sidenav-link'
      );
      const { PharosSidenavMenu } = await import(
        '@ithaka/pharos/lib/react-components/sidenav/pharos-sidenav-menu'
      );
      ...
      const isExpanded = (root: string) => {
        return (
          ...
          window.location.pathname.startsWith(withPrefix(`/${root}`))
        );
      };
