import { Story, Canvas, Meta, ArgsTable } from '@storybook/addon-docs';
import { html } from 'lit';
import { GuidelineLink } from '@config/GuidelineLink';

<Meta title="Forms/Input Group" />

# Input Group

<GuidelineLink path="input-group" />

<Canvas>
  <Story name="Base">
    {html`
      <div style="display: grid; grid-gap: 1rem; grid-template-columns: 524px;">
        <pharos-input-group name="my-input-group">
          <span slot="label">Search</span>
          <pharos-button
            name="search-button"
            icon="search"
            variant="subtle"
            label="search"
          ></pharos-button>
        </pharos-input-group>
      </div>
    `}
  </Story>
</Canvas>

## API

<ArgsTable of="pharos-input-group" />

# Cozy

<Canvas>
  <Story name="Cozy">
    {html`
      <div style="display: grid; grid-gap: 1rem; grid-template-columns: 524px;">
        <pharos-input-group name="my-input-group" variant="cozy">
          <span slot="label">Basic</span>
          <pharos-button
            name="search-button"
            icon="search"
            variant="subtle"
            label="search"
          ></pharos-button>
        </pharos-input-group>
        <pharos-input-group name="my-input-group" variant="cozy" invalidated>
          <span slot="label">invalidated</span>
          <pharos-button
            name="search-button"
            icon="search"
            variant="subtle"
            label="search"
          ></pharos-button>
        </pharos-input-group>
        <pharos-input-group name="cozy-prepend" variant="cozy">
          <pharos-button
            name="book-button"
            slot="prepend"
            icon="book"
            variant="subtle"
            label="book"
          ></pharos-button>
          <span slot="label">Cozy Prepend</span>
        </pharos-input-group>
        <pharos-input-group name="cozy-buttons" variant="cozy">
          <pharos-button
            name="book-button"
            slot="prepend"
            icon="book"
            variant="subtle"
            label="book"
          ></pharos-button>
          <span slot="label">Multiple buttons</span>
          <pharos-button
            name="close-button"
            icon="close"
            variant="subtle"
            label="close"
          ></pharos-button>
          <pharos-button
            name="search-button"
            icon="search"
            variant="subtle"
            label="search"
          ></pharos-button>
        </pharos-input-group>
      </div>
    `}
  </Story>
</Canvas>

# Validity

<Canvas>
  <Story name="Validity">
    {html`
      <div style="display: grid; grid-gap: 1rem; grid-template-columns: 524px;">
        <pharos-input-group invalidated value="not an email" name="my-input-group">
          <span slot="label">Email</span>
          <pharos-button
            name="search-button"
            icon="search"
            variant="subtle"
            label="search"
          ></pharos-button>
        </pharos-input-group>
        <pharos-input-group validated value="here@there.com" name="my-input-group">
          <span slot="label">Email</span>
          <pharos-button
            name="search-button"
            icon="search"
            variant="subtle"
            label="search"
          ></pharos-button>
        </pharos-input-group>
      </div>
    `}
  </Story>
</Canvas>

# Composition

<Canvas>
  <Story name="Composition">
    {() => {
      return html`
        <div style="display: grid; grid-gap: 1rem; grid-template-columns: 524px;">
          <pharos-input-group name="my-input-group-icon">
            <span slot="label">With icons</span>
            <pharos-icon id="calendar" name="calendar"></pharos-icon>
          </pharos-input-group>
          <pharos-input-group name="my-input-group-buttons">
            <span slot="label">Multiple buttons</span>
            <pharos-button
              name="close-button"
              icon="close"
              variant="subtle"
              label="close"
            ></pharos-button>
            <pharos-button
              name="search-button"
              icon="search"
              variant="subtle"
              label="search"
            ></pharos-button>
          </pharos-input-group>
          <pharos-input-group name="my-input-group-select">
            <span slot="label">With select</span>
            <pharos-input-group-select name="my-group-select" hide-label>
              <span slot="label">Search</span>
              <option value="">Search all content</option>
              <option value="book">Search within this book</option>
            </pharos-input-group-select>
            <pharos-button
              name="search-with-select-button"
              icon="search"
              variant="subtle"
              label="search"
            ></pharos-button>
          </pharos-input-group>
          <pharos-input-group name="my-input-group-prepend">
            <pharos-button
              name="book-button"
              slot="prepend"
              icon="book"
              variant="subtle"
              label="book"
            ></pharos-button>
            <span slot="label">Prepend</span>
          </pharos-input-group>
        </div>
      `;
    }}
  </Story>
</Canvas>
