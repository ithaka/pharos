...
<Canvas>
  <Story name="One column">{<PharosGrid>{gridColumns()}</PharosGrid>}</Story>
</Canvas>
  ...
  <Story name="Two column">
    {
      <Fragment>
        <PharosGrid layout="2-col">
          <div className="grid-example__container--first"></div>
          <div className="grid-example__container--second"></div>
          <div className="grid-example__container--third"></div>
        </PharosGrid>
...
<Canvas>
  <Story name="Column layouts">
    {
      <PharosGrid style={{ margin: '1rem 0' }}>
        {gridItems([1, 2, 3, 4, 6])}
        {comboItems([7, 8, 9, 10])}
      </PharosGrid>
    }
