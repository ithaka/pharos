...
export const renderCssAttributes = (attributes) => {
  let cssHTML = attributes.map((element) => {
    if (element.indexOf(':') != -1) {
      ...
      return html` <div style="background-color: ${tokens.color['Marble gray']['97'].value};">
        <code
          style="color:${tokens.color['JSTOR red'].value}; background-color:${tokens.color[
            'Marble gray'
          ]['97'].value};"
          >${property}</code
        ><code style="background-color:${tokens.color['Marble gray']['97'].value}"
          >${remaining}</code
        >
        <br />
      </div>`;
      ...
      return html` <div style="background-color:${tokens.color['Marble gray']['97'].value};">
        <code style="background-color:${tokens.color['Marble gray']['97'].value}">${element}</code>
        <br />
      </div>`;
    }
  });
