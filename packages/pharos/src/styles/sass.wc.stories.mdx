  ...
  let cssHTML = attributes.map((element) => {
    if (element.indexOf(':') != -1) {
      ...
      return html` <code
          ...
          >${property}</code
        ><code style="background-color:var(--pharos-color-marble-gray-97);">${remaining}</code>
        <br />`;
    } else {
      return html` <code style="background-color:var(--pharos-color-marble-gray-97);>${element}</code>
        <br />`;
    }
  });
  return cssHTML;
        ...
        ${Object.keys(sass_values.mixins).map((key) => {
          return html`
            <tr>
              ...
              <td>${sass_values.mixins[key].category}</td>
              <td>${sass_values.mixins[key].usage}</td>
              <td>
                <div
                  style="background-color:var(--pharos-color-marble-gray-97); border-style: solid; border-color: var(--pharos-color-marble-gray-80); padding-bottom:var(--pharos-spacing-one-eighth-x)"
                >
