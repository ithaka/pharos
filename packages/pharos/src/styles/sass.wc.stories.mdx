...
export const renderCssAttributes = (attributes) => {
  let cssHTML = attributes.map((element) => {
    if (element.indexOf(':') != -1) {
      ...
      return html` <div style="background-color:#F5F5F5">
        <code style="color:blue; background-color:#F5F5F5">${property}</code
        ><code>${remaining}</code> <br />
      </div>`;
    } else {
      return html` <div style="background-color:#F5F5F5">
        <code style="background-color:#F5F5F5">${element}</code> <br />
      </div>`;
    }
  });
  return cssHTML;
