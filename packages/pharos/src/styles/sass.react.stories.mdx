...
export const renderCssAttributes = (attributes) => {
  let cssHTML = attributes.map((element, index) => {
    if (element.indexOf(':') != -1) {
      ...
      return (
        <div key={index} style={{ backgroundColor: tokens.color['Marble gray']['97'].value }}>
          <code
            style={{
              color: tokens.color['JSTOR red'].value,
              backgroundColor: tokens.color['Marble gray']['97'].value,
            }}
          >
            {property}
          </code>
          <code style={{ backgroundColor: tokens.color['Marble gray']['97'].value }}>
            {remaining}
          </code>
          <br />{' '}
        </div>
