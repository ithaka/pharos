...
export const renderCssAttributes = (attributes) => {
  let cssHTML = attributes.map((element, index) => {
    if (element.indexOf(':') != -1) {
      ...
      return (
        <div key={index} style={{ backgroundColor: 'var(--pharos-color-marble-gray-94)' }}>
          <code
            style={{
              color: 'var(--pharos-color-jstor-red)',
              backgroundColor: 'var(--pharos-color-marble-gray-94)',
            }}
          >
            {property}
          </code>
          <code style={{ backgroundColor: 'var(--pharos-color-marble-gray-94)' }}>{remaining}</code>
          <br />{' '}
        </div>
      );
    } else {
      return (
        <div key={index} style={{ backgroundColor: 'var(--pharos-color-marble-gray-94)' }}>
          <code style={{ backgroundColor: '-var(--pharos-color-marble-gray-94)' }}>{element}</code>{' '}
          <br />
        </div>
      );
    }
