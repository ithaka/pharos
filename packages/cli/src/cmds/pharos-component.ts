...
const createPharosComponent = async (componentName: ComponentName): Promise<void> => {
  ...
  const baseFilePath: FilePath = `${currentDirectory}/packages/core/src/components/${componentName}`;
  ...
  if (!wasError) {
    ...
    console.log('\n@pharos/cli encountered an error attempting to create your component\n'.red);
  }
};
